...
const CharacterCounter = ({ current, limit, label }: { current: number; limit: number; label: string }) => {
  ...
  // 使用背景色
  const colorClass = isOver ? "bg-red-500" : isClose ? "bg-yellow-500" : "bg-green-500";
  // 使用文字色
  const textColorClass = isOver ? "text-red-500" : isClose ? "text-yellow-600" : "text-green-600";
  const percent = Math.min((current / (limit || 1)) * 100, 100);
