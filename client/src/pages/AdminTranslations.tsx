...
import { Loader2, Sparkles, Save, CheckCircle2, AlertCircle, ScanSearch, Plus, Trash2 } from "lucide-react";
...
export default function AdminTranslations() {
  ...
  const createMutation = trpc.admin.translations.create.useMutation({
    onSuccess: () => {
      toast.success('新增成功');
  ...
  const deleteMutation = trpc.admin.translations.delete.useMutation({
    onSuccess: () => {
      toast.success('刪除成功');
      refetch();
    },
  ...
  const handleCreate = () => {
    ...
    createMutation.mutate({ key: newKey, zhTW: newValue });
  };

  const handleDelete = (key: string) => {
    if (confirm(`確定要刪除翻譯 Key: ${key} 嗎？此操作會同時刪除所有語言的對應翻譯。`)) {
                  ...
                  <CardContent>
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                        ...
                        <DialogContent>
                          <DialogHeader>
                            <DialogTitle>新增翻譯條目</DialogTitle>
                            <DialogDescription>
                              新增一個新的翻譯 Key 和繁體中文對照。
                            </DialogDescription>
                          </DialogHeader>
                          <div className="space-y-4 py-4">
                            <div className="space-y-2">
                              ...
                              />
                              <p className="text-xs text-gray-500">建議使用點號分隔，如 category.item</p>
                            </div>
                            <div className="space-y-2">
                              <Label htmlFor="zhTW">繁體中文</Label>
                              <Input
                                id="zhTW"
                                placeholder="請輸入中文內容"
                                value={newValue}
