...
import { subscribers, partners, siteSettings, productModels, faqs, warrantyRegistrations, supportTickets, ticketReplies, seoSettings, jobs } from "../../drizzle/schema";
...
export const adminRouter = router({
  ...
  seo: router({
    ...
    getSitemapStats: publicProcedure.query(async () => {
...
Example: If input is "Apolnus® Air Purifier", output must be:
{
  ...
  "zh-CN": "Apolnus® 空气净化器"
    ...
    batchFillProductSeo: publicProcedure
      .mutation(async () => {
        const db = await getDb();
        if (!db) throw new Error("Database not available");
    ...
    // Refresh Sitemap
    refreshSitemap: publicProcedure
      .mutation(async () => {
...
Example output: ["Air Purifier", "Product Specifications"]`;
    ...
    extractFromSource: publicProcedure
      .mutation(async () => {
